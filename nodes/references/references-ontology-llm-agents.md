...
## Practitioner build-outs (single-source — label accordingly)

- **"How to Implement a Unified Memory From Scratch" (Agent Memory From Scratch)** — Paul Iusztin, *Decoding AI* (2026-07; **MongoDB-sponsored**). **[practitioner / vendor-adjacent — one engineer's opinionated design, not evaluated]** A concrete end-to-end build of an ontology-grounded unified memory in a single store. Ideas worth borrowing: the ontology defined *once* (Pydantic → `model_json_schema()`) as **"a contract between the writer and the reader,"** injected into *both* the extraction prompt and the query prompt ("1 artifact, 2 consumers, no drift"); POLE+O entity model (Person/Object/Location/Event/Organization, Neo4j Labs) + `fact` (an "atomic subject-predicate-object triplet") + `preference` (typed slots); three memory layers (long-term KG / short-term conversation+session / reasoning agent+tool_call+memory); a **7-stage write path** — chunk → extract → validate → resolve-names → embed → dedupe → upsert — with **content-derived IDs** (`{user_id}:{type}:{name}`, `{source}|{type}|{target}`) so writes are idempotent, and an embedding-dedup **band** (≥0.95 cosine merges / ≤0.85 new node / 0.85–0.95 flagged via a `same_as` edge for human review, because **"a wrong merge is the only unrecoverable mistake"**); `valid_from`/`valid_until` + `superseded_by` temporality (valid-time + supersession — *not* full bi-temporal); hybrid `$vectorSearch` + `$text` fused via **Reciprocal Rank Fusion** (Cormack, Clarke, Büttcher, SIGIR 2009) then a 2-hop `$graphLookup`. Names Graphiti/mem0/cognee/Zep as prior art. Treat its per-vendor choices and thresholds as *design defaults*, not measured optima.
