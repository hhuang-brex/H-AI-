...
- **LEGO-RL: Harness-Native Reinforcement Learning for Coding Agents** — Du, Jiang, Yuan, Dai, Wang, Chen, Tao, Yu, Shang, Wong, Li, Bai (2026-08-18). https://arxiv.org/abs/2608.17393 — The harness as *training substrate*: bridges a native coding-agent harness to policy-gradient optimization "without modifying their internal control flow," via in-process LLM proxying that captures raw generation streams for token-level credit. Its diagnosis is what to cite: native harness environments are "inherently misaligned with policy-gradient training" because environmental crashes and reward hacking corrupt outcome signals, while train-inference discrepancies decouple rollout behavior from policy updates. Adds a fourth grain to [harness-as-hyperparameter](../concepts/harness-as-hyperparameter.md) — weights move, harness held still. Author-reported; webpage `lego-rl.pages.dev`.

- **Beyond Prompts: Measuring and Optimizing LLM Tool-Agent Harnesses** — Zhao, Ruan, Chen, Tu, Abbasi, Hesch (2026-09-04, rev. 2026-09-09; **EMNLP 2026**). https://arxiv.org/abs/2609.05736 — The measurement protocol this cluster lacked. Frames the problem as **resource-bounded harness selection for fixed-model multi-turn tool agents**, with the search surface deliberately scoped to prompts and **tool-boundary middleware** — "guarded intercepts at the tool boundary, not arbitrary rewriting of agent execution logic." Reports mean held-out lift, **worst-condition lift, repeatability**, cost diagnostics, and **RelLift95(B)**, a conservative estimate of the held-out gain of the harness *selected under budget B*, motivated explicitly by the fact that "some search procedures can occasionally find large gains but still choose brittle updates." PRISM (failure clustering → repairs routed to prompt / middleware / joint surfaces in a Pareto search) reports **14.2 / 14.9 / 10.1 pp** mean held-out lift on BFCL multi-round, τ²-Retail and τ²-Telecom with positive empirical RelLift95 on all three; the ablation credits **failure-surface routing** and the edit-pattern constraint. *Verified 2026-09-11.*

- **Scanning the Harness: An Empirical Study of Supply-Chain Defects in AI Coding-Agent Configurations** — Kapner, Soceanu, Petrunin, Gartner (2026-09-07). https://arxiv.org/abs/2609.07360 — The harness as an *unaudited dependency layer* rather than a tuning surface: over 3,171 public repositories, **16.0% of multi-component setups carry a byte-decidable security defect** (unpinned MCP servers, scoped-looking blanket execution grants, skills pre-approving the installer's shell). Also a cautionary instrument note — a raw scanner rate of 25.5% fell to 16.7% after validation. Full entry in [references-agent-skill-authoring](references-agent-skill-authoring.md). *Verified 2026-09-11.*
