...
Fan-out of one seed survey. The survey's own text was read from its **arXiv LaTeX source** (`arxiv.org/src/2503.22458`), not from a summary, and each anchor below was then verified at its own primary source on **2026-08-06**. Where the survey and a primary source disagree, the primary source wins and the disagreement is recorded. Four 2026 additions at the end of this file were fetched and verified on **2026-09-11**; they are author-reported preprints/Findings papers beyond the May-2026 knowledge cutoff, none independently reproduced.
...
## 2026 additions — what the survey's taxonomy did not yet have (verified 2026-09-11)

**The history is part of the prompt.**

- **What if LLMs Ate Their Words: Causal History Effects in Multi-Turn Interaction** — Jinnan Li, Zheren Fu, Yue Wang, Jinzhe Li, Yuan Wu, Yi Chang (2026-09-05; 11 pages, no venue stated). https://arxiv.org/abs/2609.05882 — Turns the FULL→SHARDED degradation finding into an *intervention*: replay the user messages already observed in a completed trajectory while editing only the **assistant-generated** history. Replacing prior assistant responses with neutral content (**neutralization**) moves downstream min-max normalized performance by **+.027 across 2,973 trajectories**; on a prespecified length-controlled subset, short and length-matched neutralization are near-identical (**+.069 vs +.068**), so context shortening does not explain the effect. **Turn Surgery** (one assistant turn at a time) finds that **63.7% of 237 selected degraded trajectories contain at least one beneficial intervention** while most tested positions are unchanged, and **48.4%** of binary tasks admit a fail-to-success reversal. Also reports that stronger one-shot performance does not imply interaction robustness, and that an open-weight case study links consequential history edits to downstream state differences but finds **task-dependent rather than universal internal signatures** — i.e. no reusable internal probe. Conclusion: **selective rather than uniform history management**. Six task families, five models. Code and data: `jinnanli/llms-ate-their-words`. Feeds [prompt-component-attribution](../concepts/prompt-component-attribution.md).

**Diagnostics that outlive aggregate accuracy.**

- **Calibration is the Bottleneck: An Action-Class Diagnostic of Multi-Turn Tool-Calling** — Kangjia Zhao, Jiajun Li, Haozhan Shen, Wei Chow, Linfeng Li, Hang Song, Lingdong Kong, Chen Zhi + 3 (11 authors; 2026-09-01; **Findings of EMNLP 2026**). https://arxiv.org/abs/2609.00949 — Decomposes multi-turn failure into two orthogonal modes — **action-class miscalibration** and **action-execution failure** — over a four-class action space (TOOL_CALL / ASK / REFUSE / CONFIRM), with a **self-revealing upper bound Acc ≤ GAR** (Gold Action Recall). The two modes surface as **bound violation** (Acc > GAR, exposing *state-grader masking* of miscalibration) and **large bound slack** (GAR ≫ Acc, localizing execution failure inside TOOL_CALL). Reported to separate heavily tool-trained families — whose standing is inflated by the masked gap — from families with context-appropriate action choice, and to show calibration is reshapable by **context-only** perturbations but heterogeneously: one perturbation moves accuracy **+11.5 vs −21.0 pp** on the same scenario across families. The cheapest thing to copy is the bound itself: it needs no new labels and it detects when your grader is hiding a failure. Code: `fbj2333/tool-calling-calibration`. Feeds [agent-trajectory-eval](../concepts/agent-trajectory-eval.md) and [confirm-before-act](../concepts/confirm-before-act.md).

**Memory, evaluated in conversation rather than by QA probe.**

- **When Users Don't Ask: Benchmarking Context-Driven Memory Retrieval in Conversational Agents** — Wen-Yu Chang, Yun-Nung Chen (2026-09-03; no venue stated). https://arxiv.org/abs/2609.03467 — Its premise is this cluster's standing complaint: memory benchmarks "evaluate memory through QA-style probing rather than in-situ conversational usage." **LOCOMO-CONV** is derived from LoCoMo with four query styles (dialog, implicit, counterfactual, composed), scoring both retrieval recall and end-to-end response quality across five memory systems. Findings: conversational framing exposes retrieval gaps QA benchmarks miss, worst on **implicit and composed** queries; multi-facet query rewriting narrows them for raw-turn memory but **not** for abstractive memory; strong retrieval does not translate into response quality; and implicit queries show **silent grounding** — memory improves contextual grounding without surfacing the gold fact, which is exactly the case a recall metric scores as a miss. Releases `supportive_memory` annotations. **Read the provenance carefully:** it inherits LoCoMo, whose own scale this graph has recorded being mis-cited — see [conversation-memory](../concepts/conversation-memory.md) — so cite LOCOMO-CONV's derivation, not LoCoMo statistics, from this entry.

**Simulated users.**

- **A Three-Tier Persona Vector for Controllable User Simulation in Agentic Evaluation** — Rahul Khedar, Eshita, Sneha Teja Sree Reddy Thondapu, Mayank Malhotra, Arup Kumar Das, Jitesh Chandra Mishra, Arun Menon, Avinash Karn + 1 (9 authors; 2026-09-08; 7 pages, "extended treatment of the persona component of StateGen, arXiv:2606.16307"). https://arxiv.org/abs/2609.08592 — Names the default failure: flat role descriptions ("you are an angry customer") "produce near-identical conversations regardless of the underlying scenario." Proposes a **23-dimension** persona vector in three tiers — 6 categorical demographics, 12 continuous behavioral traits sampled with Gaussian noise around curated base vectors, and 5 **scenario-reactive** emotional states — plus an **orthogonal 4-level query-complexity overlay** controlling phrasing from direct to deliberately vague. Across **64,698 multi-turn conversations**, 8 named profiles, 3 production corpora: a **15.8 pp spread** in agent goal-achievement across personas, the same persona behaving differently across scenarios, ~15–20 pp gaps on booking-flow compliance between tier-aware and pressure-test personas, and seven rule-described trait correlations giving auditable co-occurrence without a learned covariance matrix. Author-reported on their own pipeline and corpora; the transferable parts are the **separation of the vagueness axis from the personality axis** and the spread-as-validity check. Feeds [simulated-user-eval](../concepts/simulated-user-eval.md).
