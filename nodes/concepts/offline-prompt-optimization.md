...
- **Most of the rollout budget buys *selection*, not learning.** "The majority of GEPA's rollout budget is spent on validation, where scores are utilized solely for candidate selection and not for producing learning signals." Counting train rollouts only, GEPA reaches optimum in **79–737** rollouts. So instrument the split before buying more budget, and note the authors' own proposed fix — smaller or **dynamically selected validation subsets**, which is state-driven allocation ([adaptive-eval-budget](adaptive-eval-budget.md), [cost-aware-eval](cost-aware-eval.md)).
...
## Does the search complexity earn its keep?

A 2026-08 preprint answers "not necessarily," and it is the sharpest challenge to this node's optimizer-choice section. **NPO (Naive Prompt Optimization)** is a deliberately minimal baseline — a **single lineage**, iteratively revised by a teacher model from rollout feedback, with no Pareto frontier, no population, and no merge. It reports **comparable or better performance than GEPA with fewer rollouts**, and, decisively, **its advantage grows with stronger teacher models** — the authors' reading being that "stronger teacher reasoning can partially substitute for optimizer-side search complexity" ([arXiv:2608.27266](https://arxiv.org/abs/2608.27266)). NPO's prompts also transfer verbatim to other student models, most reliably within a family — the same transfer property GEPA reports.

Two honest boundaries. On interactive games NPO is "broadly competitive" rather than better, and GRPO still wins on tasks less amenable to prompt optimization at all. And the authors call their own results **preliminary**; this is a 22-page preprint with no venue, not an independent replication of a GEPA ablation.

What to do with it: **run the naive single-lineage loop as your baseline before adopting an optimizer framework.** If a strong teacher plus linear revision matches the Pareto machinery on your task, the machinery is cost you are paying for nothing — and if it does not, you now have the comparison that justifies it. The claim it contests specifically is the "candidate selection carries the gains" finding in [worked-example-gepa-mechanism](../projects/worked-example-gepa-mechanism.md); the graph carries the disagreement rather than resolving it, since both sides are author-reported on their own benchmark selections.
...
- **Reward hacking.** The agent games a weak judge. Weight programmatic checks highest; review final transcripts by hand. The reported production failures are more mundane and more alarming than adversarial gaming: **cached answer keys in the environment** (100% pass rate at 68% true capability), a **corrupted ground-truth label** that led the optimizer to delete correct compliance rules, and a **syntactically broken prompt promoted** because a silent parser fallback improved the metric ([arXiv:2609.02246](https://arxiv.org/abs/2609.02246)). The structural answer is acceptance checks that **outrank** the judge plus canary cases where a perfect score is evidence of cheating — [llm-as-judge](llm-as-judge.md).
