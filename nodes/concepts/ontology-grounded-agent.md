...
An ontology gives the agent a **shared vocabulary** across the user's utterance, the agent's [run-state-model](run-state-model.md), its memory, and its actions — so "the review" resolves to the same typed entity everywhere, and the agent can reason about dependencies, states, and time instead of re-parsing prose each turn. A sharp practitioner framing of *why the vocabulary must be a single artifact*: treat the ontology as **a contract between the writer and the reader** — define it once (e.g. Pydantic → JSON Schema) and inject the *same* schema into both the extraction/write prompt and the query/read prompt, so there is **one artifact, two consumers, no drift** (Iusztin, *Decoding AI*, 2026 — practitioner/vendor-sponsored; see [references-ontology-llm-agents](../references/references-ontology-llm-agents.md)). If the write path and read path drift onto different schemas, grounding silently breaks. But two failure modes are documented and recurring:
