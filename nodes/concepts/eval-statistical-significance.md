...
6. **Report the reliability of the candidate you picked, not just the average lift.** A search can find a large gain and still select a *brittle* configuration. The protocol worth copying reports mean held-out lift **plus worst-condition lift, plus repeatability, plus a conservative budget-indexed estimate** of the selected candidate's held-out gain (RelLift95(B)) — the explicit motivation being that "some search procedures can occasionally find large gains but still choose brittle updates" ([arXiv:2609.05736](https://arxiv.org/abs/2609.05736), EMNLP 2026). This is the winner's curse with an instrument attached: the selection procedure, not only the winner, is what you are reporting on. See [harness-as-hyperparameter](harness-as-hyperparameter.md).
7. **Gate on two conditions, not one.** Ship if the change (a) beats baseline by more than the spread *and* (b) breaks zero critical cases. Criterion (b) is not statistical — a single safety regression is disqualifying at any p-value.
