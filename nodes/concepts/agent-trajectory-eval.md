...
**Action-class calibration, separated from execution.** Aggregate multi-turn tool-calling accuracy averages over situations that call for different *kinds* of action. Decomposing a four-class action space (`TOOL_CALL` / `ASK` / `REFUSE` / `CONFIRM`) yields a self-revealing bound, **Acc ≤ GAR** (Gold Action Recall): a violation (Acc > GAR) exposes a **state grader masking miscalibration**, while large slack (GAR ≫ Acc) localizes the failure *inside* `TOOL_CALL` ([arXiv:2609.00949](https://arxiv.org/abs/2609.00949), Findings of EMNLP 2026). Calibration is reshapable by context-only perturbations, but not portably: one perturbation moved accuracy **+11.5 pp for one model family and −21.0 pp for another on the same scenario**. For a confirm-before-act agent this is the diagnostic that separates "chose to act when it should have asked" from "asked correctly, then called the wrong tool" — see [confirm-before-act](confirm-before-act.md).
